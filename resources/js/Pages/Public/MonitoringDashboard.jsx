            ...
            <section className="max-w-3xl mx-auto px-6 py-12">
                <div className="bg-white rounded-xl border border-brume shadow-sm p-8">
                    <div className="flex items-center justify-between gap-4 flex-wrap">
                        ...
                        <span className={
                            ...
                            (subscription.active
                                ...
                                : 'bg-papier border-brume text-acier')
                        }>
                    ...
                    <dl className="mt-6 grid gap-4 md:grid-cols-2 text-sm">
                        <div><dt className="text-acier">Email de contact</dt><dd className="mt-1">{subscription.email}</dd></div>
                        <div><dt className="text-acier">Actif jusqu'au</dt><dd className="mt-1">{subscription.active_until ? new Date(subscription.active_until).toLocaleDateString('fr-FR') : '—'}</dd></div>
                        <div><dt className="text-acier">Dernier audit</dt><dd className="mt-1">{subscription.last_run_at ? new Date(subscription.last_run_at).toLocaleString('fr-FR') : '— (en attente)'}</dd></div>
                        <div><dt className="text-acier">Dernier score</dt><dd className="mt-1 font-semibold">{subscription.last_score_total ?? '—'}/100</dd></div>
                    </dl>
                    ...
                    <p className="mt-8 text-xs text-acier">
