...
function Metric({ label, value, hint }) {
    return (
        <div className="bg-white rounded-lg border border-brume p-5 shadow-sm text-center">
            <div className="text-xs uppercase tracking-wide text-acier">{label}</div>
            <div className="mt-2 text-2xl font-bold">{value ?? '—'}</div>
            {hint && <div className="mt-1 text-xs text-acier">{hint}</div>}
        </div>
    );
}
...
export default function AuditCwv({ meta, audit }) {
    ...
    const scoreColor =
        score === null || score === undefined ? 'text-acier'
                        ...
                        <>
                            <div className="mt-8 flex items-center justify-center">
                                <div className={`text-7xl font-bold ${scoreColor}`}>
                                    {score ?? '—'}
                                    <span className="text-2xl text-acier ml-1">/100</span>
                                </div>
                            </div>
                            <p className="text-center text-acier text-sm mt-1">
                            ...
                            <p className="mt-10 text-xs text-acier text-center">
