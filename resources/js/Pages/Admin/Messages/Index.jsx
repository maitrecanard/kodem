                    ...
                    <tbody>
                        {messages.data.map((m) => (
                            <tr key={m.id} className="border-t border-slate-100">
                                ...
                                <td className="px-4 py-2"><span className="text-xs rounded-full bg-brume px-2 py-0.5">{m.status}</span></td>
                                <td className="px-4 py-2 text-acier">{m.created_at?.slice(0, 10)}</td>
                            </tr>
                        ))}
                        {messages.data.length === 0 && (
                            <tr><td colSpan={4} className="px-4 py-6 text-center text-acier">Aucun message.</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
