...
export default function Show({ audit }) {
    return (
        <AdminLayout title="Audit">
            <div className="bg-white rounded-lg border border-brume p-6 shadow-sm">
                <div className="text-sm text-acier">{audit.uuid}</div>
                ...
                <pre className="mt-6 bg-encre text-slate-100 text-xs p-4 rounded overflow-x-auto">{JSON.stringify(audit.results, null, 2)}</pre>
            </div>
        </AdminLayout>
    );
}
